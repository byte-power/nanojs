...
func symbol(
	name string,
	scope nanojs.SymbolScope,
	index int,
) *nanojs.Symbol {
	return &nanojs.Symbol{
		Name:  name,
		Scope: scope,
		Index: index,
	}
...
func globalSymbol(name string, index int) *nanojs.Symbol {
	return symbol(name, nanojs.ScopeGlobal, index)
}

func localSymbol(name string, index int) *nanojs.Symbol {
	return symbol(name, nanojs.ScopeLocal, index)
}

func freeSymbol(name string, index int) *nanojs.Symbol {
	return symbol(name, nanojs.ScopeFree, index)
}

func symbolTable() *nanojs.SymbolTable {
	return nanojs.NewSymbolTable()
}

func resolveExpect(
	t *testing.T,
	symbolTable *nanojs.SymbolTable,
	name string,
	expectedSymbol *nanojs.Symbol,
	expectedDepth int,
) {
