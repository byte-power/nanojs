...
` + fmt.Sprintf("out = fib(%d)", n)
	...
	if nativeResult != int(result.(*nanojs.Int).Value) {
		panic(fmt.Errorf("wrong result: %d != %d", nativeResult,
			int(result.(*nanojs.Int).Value)))
	}
...
func compileFile(file *parser.File) (time.Duration, *nanojs.Bytecode, error) {
	symTable := nanojs.NewSymbolTable()
	symTable.Define("out")

	start := time.Now()

	c := nanojs.NewCompiler(file.InputFile, symTable, nil, nil, nil)
...
) (time.Duration, nanojs.Object, error) {
	globals := make([]nanojs.Object, nanojs.GlobalsSize)

	start := time.Now()

	v := nanojs.NewVM(bytecode, globals, -1)
