...
func CompileAndRun(
	...
	machine := nanojs.NewVM(bytecode, nil, -1)
	err = machine.Run()
	return
}

// RunCompiled reads the compiled binary from file and executes it.
func RunCompiled(modules *nanojs.ModuleMap, data []byte) (err error) {
	bytecode := &nanojs.Bytecode{}
...
// RunREPL starts REPL.
func RunREPL(modules *nanojs.ModuleMap, in io.Reader, out io.Writer) {
	stdin := bufio.NewScanner(in)
	fileSet := parser.NewFileSet()
	globals := make([]nanojs.Object, nanojs.GlobalsSize)
	symbolTable := nanojs.NewSymbolTable()
	...
	globals[symbol.Index] = &nanojs.UserFunction{
		Name: "println",
		Value: func(args ...nanojs.Object) (ret nanojs.Object, err error) {
			var printArgs []interface{}
			for _, arg := range args {
				if _, isUndefined := arg.(*nanojs.Undefined); isUndefined {
	...
	for {
		...
		file = addPrints(file)
		c := nanojs.NewCompiler(srcFile, symbolTable, constants, modules, nil)
		...
		bytecode := c.Bytecode()
		machine := nanojs.NewVM(bytecode, globals, -1)
...
func compileSrc(
	...
	c := nanojs.NewCompiler(srcFile, nil, nil, modules, nil)
...
func doHelp() {
	fmt.Println("Usage:")
	fmt.Println()
	fmt.Println("	nanojs [flags] {input-file}")
	fmt.Println()
	fmt.Println("Flags:")
	fmt.Println()
	...
	fmt.Println("	nanojs")
	fmt.Println()
	fmt.Println("	          Start Nanojs REPL")
	fmt.Println()
	fmt.Println("	nanojs myapp.js")
	fmt.Println()
	fmt.Println("	          Compile and run source file (myapp.js)")
	fmt.Println("	          Source file must have .js extension")
	fmt.Println()
	fmt.Println("	nanojs -o myapp myapp.js")
	fmt.Println()
	fmt.Println("	          Compile source file (myapp.js) into bytecode file (myapp)")
	fmt.Println()
	fmt.Println("	nanojs myapp")
	fmt.Println()
