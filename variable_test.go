...
func TestVariable(t *testing.T) {
	vars := []VariableTest{
		...
		},
		{
			...
			StringValue: "52.11",
			BoolValue:   true,
			Object:      &nanojs.String{Value: "52.11"},
		},
		{
			Name:        "c",
			Value:       true,
			...
			BoolValue:   true,
			StringValue: "true",
			Object:      nanojs.TrueValue,
		},
		{
			Name:        "d",
			Value:       nil,
			ValueType:   "undefined",
			Object:      nanojs.UndefinedValue,
			IsUndefined: true,
		},
	}

	for _, tc := range vars {
		v, err := nanojs.NewVariable(tc.Name, tc.Value)
